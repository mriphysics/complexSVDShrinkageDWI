...
%   multiply to the size of dimensions given by ORDIM. If empty it corresponds
%   to the size of the input array for dimension ORDIM
...
if ~exist('M','var') || isempty(M);M=N(orDim(orDim<=NDor));M=[M ones(sum(orDim>NDor))];end
if ~exist('deDim','var');deDim=[];end
