...
for m=1:nDimsOu
    if Nor(m)~=Nres(m)
        ...
        if ~quick || fo~=0 || mirror(m)==2 || comp
            if fo~=2 && mirror(m)~=2;x=fftshift(x,m);end        
            ...
            if ~fo
                if mirror(m)==2;x=ifctGPU(x,m,gpuF)*sqrt(NresM(m));else x=ifftGPU(x,m,gpuF)*NresM(m);end%IFCT NOT INCLUDED                 
            end
        else
        ...
        if mirror(m)~=2;x=mirroring(x,mirrorin==1,0);end%MIRRORING FUNCTION NOT INCLUDED
        mirrorin(m)=0;
    end
end
